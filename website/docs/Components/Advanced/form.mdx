...
| `checkbox`     | `boolean`                         | `isRequired` is only satisfied when checked                     |
...
### Configuring every field of a type (`fieldProps`)

`variant` and `color` are single values shared with the action buttons. When you need more than that — a size, a
placeholder behaviour, per-slot classes — `fieldProps` carries a full set of props per field type:
...
Keys: `input`, `selector`, `autocomplete`, `number`, `inputsGroup`, `checkbox`. Each accepts every prop of the
matching primitive except the ones the form drives itself (`value`, `onValueChange`, `error`, `label`,
`isDisabled`, `isFormControlled`, `name`, `className`).

The `props` of a single field win over `fieldProps`, and objects such as `classNames` are merged slot by slot. A
field that declares its own `preset` ignores `fieldProps` entirely — see
[presets](../../Customize/presets.md#against-the-props-of-a-single-field).
...
- **`isResetOnSubmit`** — after a submit that actually went through. A failed validation never reaches `onSubmit`,
  and a rejected `onSubmit` leaves everything in place, so the user never loses what they typed because of an error.
- **`isResetOnCancel`** — when the cancel button is pressed, before your own `onCancel` runs.
...
| `checkboxField`     | rendered `checkbox` fields             |
...
| `description`            | `ReactNode`                                    | —       | Optional text or markup under the title                  |
| `actions`                | `FormActionsConfig`                            | —       | Labels, `onCancel`, `isSubmitButtonHidden`, button props |
| `fieldProps`             | `FormFieldPropsByType`                         | —       | Props applied to every field of a given type (see below) |
