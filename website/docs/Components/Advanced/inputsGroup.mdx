...
- `initialValues` seeds the list once: `Array<{ isRequired?: boolean; value }>`. Read at mount only so later changes are ignored (uncontrolled).
...
| `initialValues`          | `Array<{ isRequired?: boolean; value }>`     | `[]`           | Seeds the list once (uncontrolled)                           |
