...
Set it once for the whole app with [`defaults.autocomplete.isInputClearedOnFocus`](../../Customize/defaults.md).
...
| `activeInputWrapper` | Extra classes on the box while `isActive`  |
...
| `triggerText`           | `string`                                                                     | -                    | Fixed text shown while the field is not focused; focusing clears the input, chips are not displayed |
| `isActive`              | `boolean`                                                                    | `false`              | Active state, drawn with a primary border                                                           |
