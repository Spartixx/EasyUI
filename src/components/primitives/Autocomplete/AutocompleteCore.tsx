...
const MULTIPLE_WRAPPER_SIZE_CLASSES: Record<NonNullable<AutocompleteProps['size']>, string> = {
  sm: 'min-h-8 py-1 px-3 gap-1.5',
  md: 'min-h-10 py-1.5 px-3 gap-2',
  lg: 'min-h-12 py-2 px-4 gap-2',
}

export interface AutocompleteCoreProps<TValue> extends AutocompleteCommonProps {
...
export function AutocompleteCore<TValue>(rawProps: AutocompleteCoreProps<TValue>) {
  ...
  const committedOption = resolvedOptions.find((option) => option.value === selectedValues[0])
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        break
      case 'Backspace':
        if (isMultiple && displayedValue === '' && selectedValues.length > 0) {
  ...
  const inputWrapper = (
    <div
      className={cn(
        ...
        isMultiple ? cn('flex-wrap', MULTIPLE_WRAPPER_SIZE_CLASSES[size]) : TEXT_FIELD_WRAPPER_SIZE_CLASSES[size],
